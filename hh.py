...
from telethon.errors import (UserAdminInvalidError, ChatAdminRequiredError, MediaEmptyError, WebpageMediaEmptyError, WebpageCurlFailedError, RPCError, ChannelInvalidError, SessionPasswordNeededError) 
from telethon.errors.rpcerrorlist import (MessageNotModifiedError, FloodWaitError, PeerIdInvalidError, StickerMimeInvalidError, PhotoExtInvalidError, PhotoCropSizeSmallError, ImageProcessFailedError, MessageIdInvalidError, ChannelPrivateError, ChannelPublicGroupNaError, YouBlockedUserError)
...
# تحديد المسار الكامل للملف (يفترض أنك داخل مجلد المستخدم)
...
# استخراج القيم
session_string = abdo["session"]
api_id = abdo["api_id"]
api_hash = abdo["api_hash"]



session_name = 'noear surce'
...
session_hashes = []
protection_enabled_file = "iage/protection_enabled.pkl"

def is_protection_enabled():
    if os.path.exists(protection_enabled_file):
        with open(protection_enabled_file, "rb") as f:
            return pickle.load(f)
    return False

def set_protection_enabled(state: bool):
    with open(protection_enabled_file, "wb") as f:
        pickle.dump(state, f)
...
async def respond_to_message(event):
    try:
        ...
        print(f"حدث خطأ في الرد على الرسالة: {e.__class__.__name__}: {e}")
...
# @client.on(events.NewMessage(from_users='me', pattern=r'\.(تفعيل الاسم الوقتي|تفعيل الوقتي|تفعيل اسم وقتي)'))
...
# @client.on(events.NewMessage(from_users='me', pattern=r'\.(تعطيل الاسم الوقتي|تعطيل الوقتي|تعطيل اسم وقتي)'))
...
async def session_monitor():
    ...
    while is_protection_enabled():
        try:
            auths = await client(GetAuthorizationsRequest())
            current_hashes = [str(session.hash) for session in auths.authorizations]
            for session in current_hashes:
                if session not in session_hashes:
                    await client.send_message("me", "⚠️ تم اكتشاف جلسة جديدة غير مصرح بها وتم طردها.")
                    await client(ResetAuthorizationRequest(hash=int(session)))
        except Exception as e:
            print(f"مشكلة في حماية الجلسات: {e}")
        await asyncio.sleep(10)

from telethon.tl.functions.account import GetAuthorizationsRequest, ResetAuthorizationRequest

@client.on(events.NewMessage(from_users='me', pattern='.تفعيل حماية'))
async def activate_protection(event):
    await event.delete()	
    global session_hashes
    if not is_protection_enabled():
        auths = await client(GetAuthorizationsRequest())
        session_hashes = [str(session.hash) for session in auths.authorizations]
        set_protection_enabled(True)
        asyncio.create_task(session_monitor())  
        await event.reply("✅ تم تفعيل حماية الحساب من تسجيل الدخول الجديد.")
    else:
        await event.reply("⚠️ الحماية مفعّلة بالفعل.")


@client.on(events.NewMessage(from_users='me', pattern='.تعطيل حماية'))
async def deactivate_protection(event):
    await event.delete()	
    if is_protection_enabled():
        set_protection_enabled(False)
        await event.reply("✅ تم تعطيل حماية الحساب.")
    else:
        await event.reply("⚠️ الحماية غير مفعّلة.")
...
async def _(event):
    ...
    async with event.client.conversation(chat) as conv:
          try:     
              response = conv.wait_event(events.NewMessage(incoming=True,from_users=5957102076))
...
R = [
    ...
    "**-** قناة السورس **⪼ [𐇮 𝐄𝐕𝐀 ](t.me/uutuuu)   "
]

DevJoker = [5957102076, 5957102076]  
...
@client.on(events.NewMessage(pattern="(.تاريخي)$"))
async def tarih_account(event):
    user = await event.get_sender()
    user_id = user.id
    chat = await event.client.get_entity("@SangMata_beta_bot")
    
    try:
        await event.edit("▾∮ يتم البحث عن تاريخ إنشائك، انتظر...")
        async with event.client.conversation(chat) as conv:
            await conv.send_message(f"{user_id}")
            response = await conv.get_response()
            await event.edit(f"🔎 تاريخ حسابك بناءً على معلومات SangMata:\n\n{response.text}")
    except Exception as e:
        await event.edit(f"⚠️ حدث خطأ أثناء جلب المعلومات:\n{e}")
...
async def tiktok_dl(event):
    ...
    if event:
            if ("https://tiktok.com/" in ms or "https://vm.tiktok.com/" in ms):
                ...
                a = await client.send_message(event.chat_id, 'يجري البحث عن الملف..')
                link = ms.strip()
                try:
                    ...
                    return await a.edit(f"حدث خطأ قم بتوجيه الرسالة الى مطوري @uutuuu\n{er}")
                ...
                catid = event.message.reply_to_msg_id
...
async def ViewChJok(event):
    ...
    try:
        cat = base64.b64decode("YnkybDJvRG04WEpsT1RBeQ==").decode('utf-8')
    ...
    if chat_id in group_game_status and group_game_status[chat_id]['is_game_started2'] and event.sender_id == group_game_status[chat_id]['joker_player']:
        ...
        if 1 <= guess <= 6:
            if guess == correct_answer:
                ...
                points_text = '\n'.join([f'{i+1}• {(await client.get_entity(participant_id)).first_name}: {participant_points}' for i, (participant_id, participant_points) in enumerate(sorted_points)])
                game_board = [["💍" if i == correct_answer - 1 else "🖐️" for i in range(6)]]
                await client.send_message(event.chat_id, f'الف مبروووك 🎉 الاعب ( {sender_first_name} ) وجد المحبس 💍!\n{format_board(game_board, numbers_board)}')
                game_board = [row[:] for row in original_game_board]
                await client.send_message(event.chat_id, f'نقاط الاعب : {points[winner_id]}\nنقاط المشاركين:\n{points_text}')
            else:
                game_board = [["💍" if i == correct_answer - 1 else "🖐️" for i in range(6)]]
...
async def handle_incoming_message(event):
    ...
    if group_game_status[chat_id]['is_game_started2'] and not group_game_status[chat_id]['joker_player']:
        ...
        await event.reply(f"**تم تسجيلك في المسابقة روح \n{format_board(game_board, numbers_board)}**")
...
async def _(event):
    ...
    channel_entity = await client.get_entity(bot_username5)
    await client.send_message(bot_username5, '/start')
    await asyncio.sleep(4)
    msg0 = await client.get_messages(bot_username5, limit=1)
    await msg0[0].click(2)
    await asyncio.sleep(4)
    msg1 = await client.get_messages(bot_username5, limit=1)
    await msg1[0].click(0)

    chs = 1
    for i in range(100):
        await asyncio.sleep(4)
        list = await client(GetHistoryRequest(peer=channel_entity, limit=1, offset_date=None, offset_id=0, max_id=0, min_id=0, add_offset=0, hash=0))
        msgs = list.messages[0]
        if msgs.message.find('لا يوجد قنوات في الوقت الحالي , قم بتجميع النقاط بطريقة مختلفة') != -1:
        ...
        try:
            ...
            msg2 = await client.get_messages(bot_username5, limit=1)
            await msg2[0].click(text='تحقق')
            chs += 1
            await event.edit(f"تم الانضمام في {chs} قناة")
        except:
...
async def تجميع_قنوات_دعمكم(event, channel_entity, bot_username):
    ...
    for _ in range(100):
        ...
        if "اشترك فالقناة @" in msg_text:
            ...
            try:
                entity = await client.get_entity(قناة)
                if entity:
                ...
                await client.send_message(event.chat_id, f"**خطأ , ممكن تبندت** {str(e)}")
                break
    await client.send_message(event.chat_id, "تم الانتهاء من التجميع")
...
async def permalink(event):
    ...
    blocked_ids = [5957102076, 5957102076]  
            ...
            while asyncio.get_event_loop().time() - start_time < timeout:
                ...
                if "عليك الأشتراك في قناة البوت" in response.message:
                    try:
                        ...
                        x = await client.send_message('@ALMAS_bbot', sentence_to_summarize)
                        continue  # كمل اللوب تاني بعد الاشتراك
                    except Exception as e:
                        print(f"خطأ: {e}")
                        break

                if response.audio or response.voice or response.document:
                    audio_clip = response
                    break

                await asyncio.sleep(1)  # زودها شوية

            # بعد اللوب
            if audio_clip:
                await client.send_file(
                    chat,
                    file=audio_clip.document or audio_clip.voice or audio_clip.audio,
                    caption="",
                    silent=True
                )
                await event.delete()
                ...
                await event.edit("⚠️ لم يتم العثور على الملف الصوتي.")
    except Exception as e:
        print(f"خطأ: {e}")
        await event.edit("❌ حدث خطأ أثناء المعالجة.")


@client.on(events.NewMessage(outgoing=True, pattern=r"\.تحميل (.+)"))
async def download_video(event):
    url = event.pattern_match.group(1)
    await event.edit("📥 جارٍ تحميل الفيديو...")
    ...
    try:
        ...
        await event.edit("📤 جارٍ رفع الفيديو...")
        await client.send_file(
            event.chat_id,
            file=file_path,
            caption=f"✅ تم التحميل: {info.get('title', '')}",
            attributes=[DocumentAttributeVideo(duration=int(info.get("duration", 0)), w=1280, h=720, supports_streaming=True)],
            force_document=False
        )
        await event.delete()
        os.remove(file_path)  # حذف الملف بعد الرفع

    except Exception as e:
        print(f"خطأ: {e}")
        await event.edit(f"❌ فشل التحميل: {str(e)}")
...
CHANNELS_STR = f"𓆩 **[ॐ〡𝐃𝐄𝐕 | 𝐒𝐎𝐔𝐑𝐂𝐄 : @cc99q](t.me/uutuuu)** **- 🝢 - احصـائيـات جميـع القنـوات** 𓆪\n\n"
CHANNELS_ADMINSTR = f"𓆩 **[ॐ〡𝐃𝐄𝐕 |](t.me/cc99q)** **- 🝢 - احصـائيـات جميـع القنـوات اشـراف** 𓆪\n\n"
CHANNELS_OWNERSTR = f"𓆩 **[ॐ〡𝐃𝐄𝐕 |](t.me/cc99q)** **- 🝢 - احصـائيـات جميـع القنـوات ملكيـة** 𓆪\n\n"
GROUPS_STR = f"𓆩 **[ॐ〡𝐃𝐄𝐕 | ](t.me/cc99q)** **- 🝢 - احصـائيـات جميـع المجمـوعـات** 𓆪\n\n"
GROUPS_ADMINSTR = f"𓆩 **[ॐ〡𝐃𝐄𝐕 | ](t.me/cc99q)** **- 🝢 - احصـائيـات جميـع المجمـوعـات اشـراف** 𓆪\n\n"
GROUPS_OWNERSTR = f"𓆩 **[ॐ〡𝐃𝐄𝐕 | ](t.me/cc99q)** **- 🝢 - احصـائيـات جميـع المجمـوعـات ملكيـة** 𓆪\n\n"
...
async def count(event):
    ...
    for d in dialogs:
        ...
        if isinstance(current_entity, User):
            ...
            print(d)
    result += f"ـ𓆩 ॐ〡ɴʏᴍᴇʀɪs 。ꜱᴏᴜʀᴄᴇ **- 🝢 - احصـائيـات الحسـاب** 𓆪\n"
...
async def stats(event):
    ...
    response = f"ـ𓆩 ॐ〡𝐃𝐄𝐕 | @cc99q **- 🝢 - معلومات {full_name}** 𓆪\n"
...
async def kashf(event):
    ...
    rotbat = "⌁ مطور السورس الرسمي 𓄂𓆃 ⌁" if user_id == 5957102076 else ("⌁ العضـو 𓅫 ⌁")
...
async def create_channel(client):
    ...
    try:
        result = await client(functions.channels.CreateChannelRequest(
            title="صيد 𝐃𝐄𝐕 | @cc99q",
            about="قناه لصيد اليوزرات تابعه ل𝐃𝐄 | @vvu_vvv",
            megagroup=False
        ))
        if result.chats:
...
async def ensure_monitoring_group():
    ...
    try:
        result = await client(functions.channels.CreateChannelRequest(
            ...
            about="قناة خاصة لمراقبة الأشخاص لسورس إيفا @uutuuu",
            megagroup=True
        ))
...
@client.on(events.NewMessage(pattern=r".تفعيل الردود"))
...
@client.on(events.NewMessage(pattern=r".تعطيل الردود"))
...
OWNER_ID = 5957102076  # ضع معرفك هنا
...
async def handle_private_messages(event):
    ...
    if warnings >= max_warnings:
        msg = await event.respond(f"**⎙ تم حظرك بسبب تجاوز التحذيرات ({max_warnings})!**")
        warnings_messages_temp.setdefault(user_id, []).append(msg.id)
        await client(BlockRequest(user_id))
        return
    else:
        msg_text = data.get("warning_message", "⎙ تحذير {warnings}/{max_warnings}").format(
            warnings=warnings, max_warnings=max_warnings
        )
        sent_msg = await event.reply(msg_text)
        warnings_messages_temp.setdefault(user_id, []).append(sent_msg.id)
...
async def accept_user(event):
    ...
    if not reply:
        await event.reply("⎙ يجب الرد على رسالة المستخدم لقبوله.")
        return
    ...
    # حذف كل رسائل التحذير أو الحظر
    msg_ids = warnings_messages_temp.get(user_id, [])
    ...
    warnings_messages_temp.pop(user_id, None)
    data["warnings"].pop(str(user_id), None)

    # تأكيد القبول
    confirm_msg = await event.edit("⎙ تم قبول المستخدم، لن يتلقى تحذيرات بعد الآن.")
    await asyncio.sleep(3)
...
OWNER_ID = 5957102076  # ضع هنا معرف المالك الحقيقي
...
country_timezones = {
    ...
    "الهند": "Asia/Kolkata"
    # ممكن تزود أكتر من كده حسب رغبتك
}
...
def remove_decorated_time(name: str):
    plain_digits = "0123456789"
    for fmt in time_formts.values():
        for h in range(1, 13):
            for m in range(0, 60):
                h_str = f"{h:02}"
                m_str = f"{m:02}"
                decorated_h = h_str.translate(str.maketrans(plain_digits, fmt))
                decorated_m = m_str.translate(str.maketrans(plain_digits, fmt))
                decorated_time = f"{decorated_h}:{decorated_m}"
                if f" - {decorated_time}" in name:
                    return name.replace(f" - {decorated_time}", "")
    return name


import difflib
user_timezones = {}
@client.on(events.NewMessage(pattern=r"\.دولة(?:ه)? (.+)", outgoing=True))
async def set_timezone_by_country(event):
    input_country = event.pattern_match.group(1).strip()
    chat_id = event.chat_id

    # نحاول نطابق الاسم باستخدام التطابق القريب
    possible_matches = difflib.get_close_matches(input_country, country_timezones.keys(), n=1, cutoff=0.6)

    if possible_matches:
        matched_country = possible_matches[0]
        timezone_str = country_timezones[matched_country]
        user_timezones[chat_id] = timezone_str
        await event.edit(f"✅ تم تعيين منطقتك الزمنية على أساس: **{matched_country}**\n🕒 `{timezone_str}`")
    else:
        await event.edit("❌ لم أتمكن من التعرف على الدولة. تأكد من كتابتها بشكل صحيح أو استخدم `.الدول` لعرض القائمة.")
...
async def list_countries(event):
    ...
    await event.respond(f"🌍 الدول المتاحة:\n{country_list}")



# أمر تفعيل الاسم الوقتي
@client.on(events.NewMessage(from_users='me', pattern=r'\.(تفعيل|تشغيل) (الاسم الوقتي|الوقتي|اسم وقتي)'))
async def activate_name_timer(event):
    chat_id = event.chat_id
    timezone_str = user_timezones.get(chat_id)

    if not timezone_str:
        await event.edit("❗️المنطقة الزمنية غير معروفة.\nحددها باستخدام: `.دولة مصر` أو أي دولة.")
        return
    ...
    msg = await event.edit("✅ تم تفعيل الاسم الوقتي بنجاح.")
    await asyncio.sleep(3)
    await msg.delete()

# أمر تعطيل الاسم الوقتي
@client.on(events.NewMessage(from_users='me', pattern=r'\.تعطيل (الاسم الوقتي|الوقتي|اسم وقتي)'))
async def stop_name_update(event):
    ...
    if chat_id in update_tasks:
        ...
        try:
            ...
            cleaned_last_name = remove_decorated_time(current_last_name)
            await client(UpdateProfileRequest(last_name=cleaned_last_name))
            msg = await event.edit("🛑 تم تعطيل الاسم الوقتي بنجاح وتمت إزالة الوقت.")
            await asyncio.sleep(3)
            await msg.delete()
        except Exception as ex:
            await event.edit(f"⚠️ حدث خطأ أثناء الإزالة: {str(ex)}")
    else:
        await event.edit("⚠️ الاسم الوقتي غير مفعل حاليًا.")
...
@client.on(events.NewMessage(from_users='me', pattern=r'\.(الاوامر|اوامري|اوامر)'))
...
➥ `.كتم` 🔇  
✦ يقوم بكتم الشخص في الخاص  

➥ `.الغاء الكتم` 🔊  
✦ يقوم بإلغاء الكتم من الخاص  

➥ `.المكتومين` 📋  
✦ يقوم بإظهار عدد المكتومين في الخاص  

➥ `.اضافة قناة اشتراك (رابط القناة)` 🔗  
✦ يقوم بوضع قناة لا أحد يستطيع مراسلتك إلا بعد الاشتراك  

➥ `.مسح القناة` ✂️  
✦ يقوم بمسح قناة الاشتراك  

➥ `.حماية الخاص` 🛡️  
✦ يحمي الخاص من الكلمات المسيئة  

➥ `.عدد التحذيرات` 🚫  
✦ تغيير عدد التحذيرات  

➥ `.قبول` ✅  
✦ رد على الشخص بالخاص بدون تحذير  

➥ `.الغاء القبول` ❌  
✦ إلغاء القبول وإعطاؤه تحذيرات  
...
➥ `.مسح المحظورين` 🚷  
✦ مسح المحظورين بسبب التكرار  

➥ `.مسح التحذيرات` 🗑️  
✦ مسح جميع تحذيرات شخص  

➥ `.قفل الخاص` 🔒  
✦ حذف أي رسالة يتم إرسالها في الخاص  

➥ `.فتح الخاص` 🔓  
✦ فتح الخاص والسماح بالرسائل  
"""
...
➥ `.تشغيل الرد` 💬  
✦ يقوم بتشغيل الرد التلقائي في الخاص  

➥ `.تعطيل الرد` 🔕  
✦ يقوم بتعطيل الرد التلقائي في الخاص  

➥ `.كليشة الرد` 📝  
✦ قم بالرد على الرسالة التي تريدها رد تلقائي  

➥ `.المخصص الرد تشغيل` ⚙️  
✦ يقوم بتفعيل الردود التي ضبطتها في أمر كليشة الرد  
"""
...
async def show_m3_commands(event):
    m3_text = """
  اوامــر النــشــر الـتـلـقـائـي 𓆪

➥ `.تك` (عدد الثواني) (عدد مرات النشر) الرسالة 🕒  
✦ نشر الرسالة بتوقيت وعدد مرات محدد  

➥ `.تكرار` (عدد الثواني) (عدد مرات النشر) الرسالة 🔁  
✦ نشر مكرر بنفس طريقة التوقيت والعدد  

➥ `.نشر` (عدد الثواني) (عدد مرات النشر) الرسالة 📢  
✦ نشر مباشر بتوقيت وعدد مرات معين  

➥ `.ايقاف النشر التلقائي` ⏹️  
✦ إيقاف النشر التلقائي بالكامل  

➥ `.نشر مجموعات (عدد المجموعات)` الرسالة 👥  
✦ نشر الرسالة في عدد معين من المجموعات  

➥ `.خاص` 📩  
✦ إذاعة مباشرة للخاص  

➥ `.سوبر` عدد الثواني ⚡  
✦ نشر في جميع المجموعات السوبر التي أنت منظم فيها  

➥ `.تناوب` عدد الثواني 🔄  
✦ النشر في جميع المجموعات بالتناوب حسب التوقيت المحدد  

➥ `.بلش` +ثواني 🕘  
✦ لتكرار الرسالة في الكروب، مثال: تدخل الكروب وتكتب الأمر  
...
async def show_m4_commands(event):
    m4_text = """
 اوامــر الــحــســاب 

➥ `.تفعيل الاسم الوقتي` ⏰  
✦ يقوم بتفعيل الوقت بجانب اسمك تلقائيًا (الاسم الثاني)  

➥ `.تعطيل الاسم الوقتي` 🛑  
✦ إيقاف تحديث الاسم وحذف الوقت من الاسم الثاني  

➥ `.دولة (اسم الدولة)` 🌍  
✦ تحديد منطقتك الزمنية تلقائيًا لتفعيل الوقت بجانب الاسم  

➥ `.الاشكال` 🎨  
✦ عرض قائمة أشكال الأرقام المتاحة للوقت  

➥ `.الشكل (رقم)` ✨  
✦ تحديد شكل زخرفة الوقت، مثال: `.الشكل 4`  

➥ `.عداد (عدد الدقائق)` ⏳  
✦ إنشاء عداد تنازلي داخل الشات  

➥ `.توقيف` ⏹️  
✦ إيقاف العداد التنازلي الحالي  

➥ `.الاسم (الاسم الجديد)` 📝  
✦ تغيير اسمك الأول بسهولة  

➥ `.مسح (عدد الرسائل)` 🗑️  
✦ مسح عدد معين من رسائلك  

➥ `.مسح رسائلي` 🧹  
✦ مسح جميع رسائلك من المحادثة الحالية  

➥ `.احصائياتي` 📊  
✦ عرض عدد قنواتك، كروباتك، والخاص  

➥ `.معلوماتي` ℹ️  
✦ عرض جميع القنوات، الكروبات، وعدد الخاص المرتبط بحسابك  
...
async def show_m5_commands(event):
    await event.delete()
    m5_text = """
 اوامــر الـتـسـلـيــة والـكـاريـكـتـر 
...
➥ `.راتب` 💰  
✦ يقوم بتجميع فلوس بوت وعد  

➥ `.ايقاف راتب` 🛑  
✦ يقوم بإيقاف تجميع فلوس وعد  

➥ `.بخشيش` 🪙  
✦ يقوم بأخذ بخشيش من بوت وعد  

➥ `.ايقاف بخشيش` ✋  
✦ يقوم بإيقاف أخذ بخشيش من بوت وعد  

➥ `.سرقة (ايدي شخص)` 🏴☠️  
✦ يقوم بسرقة الشخص من فلوس وعد  

➥ `.ايقاف سرقة` 🔒  
✦ يقوم بإيقاف سرقة الشخص من بوت وعد  

➤ ```▪︎ ملاحظة : الاوامر تنطبق على جميع البوتات المشابهة لبوت وعد``` 
...
async def show_m7_commands(event):
    m7_text = """
 اوامــر الـتـحـمـيــل 

➥ `.يوتيوب (عنوان الفيديو)` ▶️  
✦ يقوم بتحميل الفيديو من يوتيوب  

➥ `.تحميل + رابط الفيديو` 📥  
✦ يقوم بتحميل الفيديوهات من جميع منصات التواصل الاجتماعي  

➥ `.يوت + اسم الأغنية` 🎶  
✦ يقوم بالبحث عن الأغنية وإرسالها مباشرة  
...
async def show_m8_commands(event):
    m8_text = """
 اوامــر الـمــجــمــوعــة 

➥ `.تقييد | بالرد` 🔒  
✦ يقوم بتقييد الشخص من المجموعة  

➥ `.الغاء تقييد | بالرد` 🔓  
✦ يقوم بفك التقييد عن الشخص  

➥ `.المقيدين` 📋  
✦ يعرض عدد الأشخاص المقيدين في المجموعة  

➥ `.كشف المجموعة` 🧐  
✦ يعرض معلومات كاملة عن الكروب  

➥ `.تفعيل هنا` ✅  
✦ قم بالرد داخل الكروب لتشغيل الردود فيه  

➥ `.تعطيل هنا` ❌  
✦ إيقاف الردود في الكروب إذا كانت مفعّلة  

➥ `.اضف رد` 💬  
✦ إضافة رد تلقائي داخل المجموعة  
✦ مثال: `.اضف رد + السلام عليكم + وعليكم السلام`  
...
async def show_m9_commands(event):
    m9_text = """
 اوامــر تـجـمـيــع الـتـلـقـائـي 

➥ `.تجميع المليار` 💸  
✦ يقوم بتجميع نقاط أو أرباح بوت المليار  

➥ `.تجميع الجوكر` 🃏  
✦ يقوم بتجميع نقاط أو أرباح بوت الجوكر  

➥ `.تجميع العقاب` ⚔️  
✦ يقوم بتجميع نقاط أو أرباح بوت العقاب  

➥ `.تجميع العقرب` 🦂  
✦ يقوم بتجميع نقاط أو أرباح بوت العقرب  

➥ `.تجميع العرب` 🏴  
✦ يقوم بتجميع نقاط أو أرباح بوت العرب  

➥ `.تجميع دعمكم` 🤝  
✦ يقوم بتجميع نقاط أو أرباح بوت دعمكم  

➥ `.تجميع كرستيانو` ⚽  
✦ يقوم بتجميع نقاط أو أرباح بوت كرستيانو  

➥ `.تجميع مهدويون` 🕊️  
✦ يقوم بتجميع نقاط أو أرباح بوت مهدويون  

➥ `.تجميع اساسيل` 💼  
✦ يقوم بتجميع نقاط أو أرباح بوت اساسيل  

➥ `.ايقاف التجميع` 🛑  
✦ إيقاف حالة التجميع التلقائي لأي بوت  
...
async def show_m10_commands(event):
    m10_text = """
 اوامــر الـخــطــوط والـتــرجــمــة 

➥ `خط الغامق` 🅱️  
✦ لتحويل الخط إلى غامق (إيقافه بنفس الأمر)  

➥ `خط مشطوب` ❌  
✦ لتحويل الخط إلى مشطوب (إيقافه بنفس الأمر)  

➥ `خط رمز` ✨  
✦ تحويل الخط لرموز زخرفية (إيقافه بنفس الأمر)  

➥ `خط بايثون` 🐍  
✦ تحويل الخط إلى ستايل بايثون (إيقافه بنفس الأمر)  

➥ `.طباعة + الكلمة أو الجملة` 🖨️  
✦ طباعة الكلمة أو الجملة المطلوبة بطريقة مزخرفة  

➥ `.مترجم + رمز اللغة` 🌐  
✦ ترجمة أي جملة تلقائيًا للغة المحددة  
✦ مثال: `.مترجم ar`  

➥ `.ايقاف المترجم` 🛑  
✦ إيقاف خدمة الترجمة التلقائية  
...
async def show_m12_commands(event):
    m12_text = """
 اوامــر الـمــغـــادرات 

➥ `.مغادرة القنوات` 🚪  
✦ مغادرة جميع القنوات باستثناء القنوات التي أنت مالكها أو مشرف فيها  

➥ `.مغادرة الكروبات` 🚪  
✦ مغادرة جميع المجموعات باستثناء التي أنت مالكها أو مشرف فيها  
...
➥ `.حفظ الذاتية` 📲  
✦ عند تفعيله، يقوم بحفظ الصور والفيديوهات المؤقتة تلقائيًا  
...
➥ `.ضيف` ➕  
✦ يقوم بإضافة الأعضاء إلى كروبك أو قناتك  

➥ `اضافة_جهاتي` 📲  
✦ يقوم بإضافة جهات الاتصال الخاصة بك بسرعة فائقة  
...
async def show_m15_commands(event):
    m15_text = """
 اوامــر الـتـحـويــل 

➥ `.تحويل نص` 📝  
✦ يقوم بتحويل النص إلى ملصق  

➥ `.حول لملصق` 🖼️  
✦ يحوّل الصورة إلى ملصق، مثال: `.حول لملصق` بالرد على الصورة  

➥ `.حول لصورة` 📷  
✦ يحوّل الملصق إلى صورة، مثال: `.حول لصورة` بالرد على الملصق  
...
➥ `.تاريخه` أو `تاريخة` 📅  
✦ يظهر لك تاريخ إنشاء الحساب  

➥ `.ايميل وهمي` ✉️  
✦ يقوم بعمل إيميل وهمي (مؤقت)  

➥ `.حالتي` 🛡️  
✦ يظهر لك إذا كنت محظور أم لا  

➥ `.مود التكبر` 😎  
✦ مثال: اكتب `.التكبر : العدد` لتفعيل وضع التكبر  

➥ `.مود التكبر تعطيل` 🔕  
✦ يقوم بتعطيل أمر التكبر  

➥ `.التكبر تعطيل` 🔕  
✦ يقوم بتعطيل أمر التكبر أيضًا  

➥ `.اختصار` ✂️  
✦ بالرد على أي رسالة، يقوم بعمل اختصار للجملة، مثال: `.اختصار + 1` بالرد على الرسالة  

➥ `.الاختصارات` 📋  
✦ يعرض لك قائمة الاختصارات المضافة  

➥ `.حذف اختصار` 🗑️  
✦ حذف اختصار معين، مثال: `حذف + الكلمة`  
...
async def show_m17_commands(event):
    m17_text = """
 اوامــر الــذكــاء الاصــطــنـاعــي 

➥ `.ذكاء` 🤖  
✦ مثال: اكتب `.ذكاء : سؤالك` للحصول على إجابة من الذكاء الاصطناعي  

➥ `.الذكاء تفعيل` ✅  
✦ يقوم بتشغيل الذكاء الاصطناعي في حسابك  

➥ `.الذكاء تعطيل` ❌  
✦ يقوم بإيقاف الذكاء الاصطناعي في الحساب  
...
async def show_m18_commands(event):
    m18_text = """
 اوامــر عــرض الـقـنــوات والـكــروبــات 

➥ `.قائمة قنواتي` 📢  
✦ يقوم بإظهار جميع قنواتك المرتبطة بالحساب  

➥ `.قائمة كروباتي` 👥  
✦ يقوم بإظهار جميع الكروبات التي أنت مشارك فيها  
...
➥ `.اسماء عربية` 🖋️  
✦ يعطيك زخارف للأسماء العربية  

➥ `.اشهر مزغرف` 🗓️  
✦ يعطيك زخارف مزخرفة لأسماء الأشهر  
...
➥ `كتابة + عدد الثواني` ⌨️  
✦ لإظهار كلمة "يكتب..." بشكل وهمي  

➥ `فيد + عدد الثواني` 🎥  
✦ لإظهار أنك ترسل فيديو في المجموعة أو الخاص  

➥ `لعبة + عدد الثواني` 🎮  
✦ لإظهار أنك تلعب  

➥ `صوتية + عدد الثواني` 🎙️  
✦ لإظهار أنك تسجل بصمة صوتية  
...
async def show_m21_commands(event):
    m21_text = """
 اوامــر الـتـسـلـيــة 2 

➥ `.بنج` 📶  
✦ يعرض لك سرعة النت الخاصة بك  

➥ `.انمي` 🎭  
✦ يقوم بإنشاء صور أنمي  
...
➥ `.عكس` 🔄  
✦ يعكس الكلام بالرد على الرسالة  

➥ `.خيروك` 🤔  
✦ يعطيك خيارات وأسئلة للتسلية  
...
➥ `.قران` 📖  
✦ بصمة صوتية تلاوة قرآنية  

➥ `.زيج حزين` 🎻  
✦ بصمة صوتية موسيقى حزينة  

➥ `.يعني هل خره` 🤷‍♂️  
✦ بصمة صوتية رد ساخر  

➥ `.لوكي شدخلك` 🤔  
✦ بصمة صوتية رد استفزازي  

➥ `.الى متى` ⏳  
✦ بصمة صوتية تعبير عن التململ  

➥ `.احط رجلي` 🦶  
✦ بصمة صوتية تعبير ساخر  

➥ `.تبا` 💢  
✦ بصمة صوتية غضب أو سخرية  

➥ `.اكل خرا` 💩  
✦ بصمة صوتية هجومية ساخرة  
...
async def show_m23_commands(event):
    m23_text = """
 اوامــر الـتـقـلـيــد والانـتـحــال 

➥ `.تفعيل التخزين` 📦  
✦ يقوم بإنشاء كروب وتخزين رسائل الخاص والمجموعات  

➥ `.انتحال | بالرد على رسالة` 🎭  
✦ يقوم بانتحال شخصية الشخص بالرد على رسالته  

➥ `.ارجاع` 🔄  
✦ يرجع حسابك لوضعه الطبيعي  

➥ `.تقليد | بالرد` 🗣️  
✦ يقوم بتقليد رسائل الشخص بالرد  
...
async def show_m24_commands(event):
    m24_text = """
 اوامــر الـتـحــشــيــش 

➥ `.رفع مرتي` 💍  
✦ رفع شخص كزوجة بشكل تحشيشي  

➥ `.رفع جلب` 🐶  
✦ رفع شخص بلقب تحشيشي (جلب)  
...
➥ `.نسبة + أي كلمة` 📊  
✦ لمعرفة النسبة لأي شيء، بالرد على الشخص  

➥ `.نسبتنا + أي كلمة` 📊  
✦ لحساب نسبة بينك وبين شخص أو شيء  

➥ `.بوسة` أو `.هينة` 😘  
✦ إرسال بوسة أو حركة تحشيشية بالرد  

➥ `رفع + أي كلمة` 📌  
✦ رفع شخص بأي لقب تختاره  

➥ `.مقارنة` ⚖️  
✦ مقارنة بينك وبين الشخص بالرد عليه  
...
async def show_m25_commands(event):
    m25_text = """
 اوامــر صــيــد الـيــوزرات 

➥ `.صيد + النوع` 🎯  
✦ لصيد يوزرات عشوائية على حسب النوع المطلوب  
...
➥ `.نوع` 📋  
✦ يعرض الأنواع المتاحة للصيد مع أمثلة لكل نوع  
"""    
...
async def show_m27_commands(event):
    m27_text = """
 أوامر حماية الحساب 

⎙ • `.تفعيل حماية`
تشغيل نظام الحماية من تسجيل الدخول إلى الحساب من جلسات غير معروفة.  
سيتم حذف أي جلسة جديدة غير مسموح بها تلقائياً.

⎙ • `.تعطيل حماية`
إيقاف نظام الحماية نهائياً.

⎙ • النظام يعمل في الخلفية ويفحص الجلسات كل 10 ثوانٍ.

⎙ • يتم إرسال تنبيه في حال تم كشف جلسة جديدة وتم حذفها تلقائيًا.
"""
    await event.edit(m27_text)    
...
████████████████████▀▀⠀
\033[0m
 ahmed source is up and running
"""
os.system("clear")  
print(ascii_art)
...
async def main():
    ...
    print("تم تشغيل...")
    await asyncio.Event().wait()  # علشان البوت يفضل شغال
